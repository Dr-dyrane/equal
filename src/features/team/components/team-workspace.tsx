...
function getSignalTone(tone: TeamSignal["tone"]) {
  if (tone === "warning") {
        ...
        <div className="grid items-start gap-4">
          <section className="story-panel px-5 py-5 sm:px-6">
            ...
            <h1 className="mt-4 text-balance font-heading text-[2.55rem] leading-[0.94] text-[color:var(--story-ink)] sm:text-[3.25rem]">
              Bring the real team into the week.
            </h1>
            <p className="mt-4 max-w-3xl text-base leading-7 text-[color:var(--story-muted)]">
              People, skills, availability, and preferences should be easy to keep
              current before the schedule starts making tradeoffs.
            </p>
        ...
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 font-heading text-[2rem] leading-none text-[color:var(--story-ink)]">
              Keep every person visible.
            </h2>
          </div>
          <span className="story-system-label">4 focus profiles</span>
        </div>

        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {teamMembers.map((member) => (
            <article key={member.name} className="story-soft-card px-4 py-4">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="story-system-label">{member.state}</span>
              </div>
