...
export function ScheduleWorkspace() {
  ...
  const days = getScheduleDays(selectedDay);
  ...
  const notes = getScheduleNotes(explanationPanelOpen);
        ...
        <div className="grid items-start gap-4">
          <section className="story-panel px-5 py-5 sm:px-6">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {focus.map((item) => (
                <span key={item.label} className="story-system-label">
          ...
          <section className="grid gap-3 md:grid-cols-3">
            {metrics.map((metric) => (
              <article key={metric.label} className="story-soft-card px-4 py-4">
                ...
                <p className={`mt-4 font-heading text-[2.25rem] leading-none ${getMetricTone(metric.tone)}`}>
                  {metric.value}
                </p>
              ...
              {notes.map((note) => (
                <div key={note.name} className="story-soft-card px-4 py-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <span className={`text-[11px] font-semibold uppercase tracking-[0.18em] ${getMetricTone(note.tone)}`}>
                      live
                    </span>
                  </div>
          ...
          {days.map((day) => {
            ...
            return (
              <button
                ...
                className="schedule-day-card p-4 text-left"
                data-active={daySelected ? "true" : "false"}
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-heading text-[1.55rem] leading-none text-[color:var(--story-ink)]">
                      {day.day}
                    </p>
                    <p className="mt-2 text-sm text-[color:var(--story-muted)]">{day.date}</p>
                  </div>
                ...
                <div className="mt-4 space-y-3">
                  {day.shifts.map((shift) => {
                    ...
                    return (
                      ...
                      >
                        <p data-schedule-meta="true" className="text-[11px] font-semibold uppercase tracking-[0.18em]">
                          {shift.time}
                        </p>
                        <p className="mt-2 text-sm font-semibold text-current">{shift.role}</p>
                        <p data-schedule-meta="true" className="mt-2 text-[11px] font-semibold uppercase tracking-[0.16em]">
                          {shift.emphasis}
                        </p>
                        <p className="mt-3 text-sm font-semibold text-current">{shift.person}</p>
                      </button>
                    );
                  })}
                </div>
